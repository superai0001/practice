# 部署手册 — cli-proxy-logger（Python 版）

本文件是 **Python 版** 的独立部署手册，自包含、与 Node/Java 版无关。涵盖：配置项速查、各环境实施步骤（本机 / 离线内网 / Linux systemd / Windows nssm / Docker / 单文件 exe）、内核出站的构建与启用。

- **运行时**：CPython ≥ 3.8（实测 3.12.8），**纯标准库零第三方依赖**（无需 `pip install`）
...
因纯标准库零依赖，内网部署只需「目录 + Python 运行时」，**不需要联网、不需要 `pip install`**。

1. **准备 Python 离线运行时**：内网备 **CPython 3.8+**——可用官方离线安装包（Windows `.exe`/嵌入式 zip、Linux 发行版系统包，Windows 免装 embeddable 包**无需 pip 装任何包**）。
2. **打包工程**：
   ```bash
   bash scripts/package.sh                                          # Linux/macOS → dist/cli-proxy-logger-py.tar.gz
   powershell -ExecutionPolicy Bypass -File scripts\package.ps1     # Windows     → dist\cli-proxy-logger-py.zip
   ```
   产出只含 `cli_proxy_logger/`、`public/`、README（自动剔除 `__pycache__`），拷进内网解压（直接拷整个目录也行）。
3. **运行**：见上面 A。

### C. 常驻后台 · Linux（systemd）

用仓库模板 `deploy/cli-proxy-logger.service`：改好路径/端口/上游，然后：

```bash
sudo cp deploy/cli-proxy-logger.service /etc/systemd/system/cli-proxy-logger-py.service
sudo systemctl daemon-reload
sudo systemctl enable --now cli-proxy-logger-py
journalctl -u cli-proxy-logger-py -f       # 看日志
```
...
# 卸载：nssm remove cli-proxy-logger-py confirm
```

临时跑：Linux `nohup python -m cli_proxy_logger > proxy.out 2>&1 &`；Windows `start /b python -m cli_proxy_logger`。

### E. Docker / docker-compose

仓库内置 `Dockerfile`（`python:3.12-slim`）+ `docker-compose.yml`。镜像内已默认 `BIND_ADDR=0.0.0.0`、`PYTHONUNBUFFERED=1`。
...
**容器内走内核**：内核二进制必须是 **Linux 版**。把 Linux 版 `xray`/`sing-box` 放进 `./vendor`，在 `docker-compose.yml` 里取消注释 `./vendor:/vendor:ro` 卷与 `XRAY_BIN`/`SING_BOX_BIN`/`PROXY_KERNEL`（不装 Go 的交叉构建命令见 compose 文件末尾注释）。

### F. 单文件 exe（PyInstaller，已实测）

本变体是纯标准库，PyInstaller 能干净地打成**免装 Python 的单文件 exe**（约 8–9 MB）。在**与目标机相同 OS** 的联网机器上：
...
运行：`set BIND_ADDR=0.0.0.0 && dist\cli-proxy-logger.exe`（Linux：`BIND_ADDR=0.0.0.0 ./dist/cli-proxy-logger`）。环境变量照常生效。
...
export UPSTREAM_PROXY='ss://<base64(method:password)>@89.45.45.120:53791#node'   # 或 vless:// 链接
...
内核子进程随主进程退出（`atexit`）自动关闭，临时配置文件自动清理。完整设计/实施过程见 [../KERNEL_INTEGRATION.md](../KERNEL_INTEGRATION.md)。
