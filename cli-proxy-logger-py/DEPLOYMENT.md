...
仓库内置 `Dockerfile`（`python:3.12-slim`）+ `docker-compose.yml`。镜像内已默认 `BIND_ADDR=0.0.0.0`、`PYTHONUNBUFFERED=1`。**镜像构建时会自动下载官方 xray + sing-box 的 Linux 二进制打进 `/app/vendor`（约 80MB），所以容器开箱即可走高级协议，无需自己准备内核**。
