...
仓库内置**多阶段** `Dockerfile`（`maven:3.9-eclipse-temurin-8` 构建 fat jar → `eclipse-temurin:8-jre` 运行）+ `docker-compose.yml`。Spring Boot 默认监听所有网卡，发布端口即可达，无需额外 `BIND_ADDR`。
...
二进制查找顺序：显式路径（`proxy.xray-bin`/`proxy.sing-box-bin` 或 `XRAY_BIN`/`SING_BOX_BIN`）→ 同级 `vendor/`（`xray`/`sing-box`，Windows 带 `.exe`）→ 系统 PATH。
